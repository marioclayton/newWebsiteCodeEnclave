...
const ServicesHero = () => {
  const [typedLines, setTypedLines] = useState(() => Array(serviceLines.length).fill(''));
  ...
  useEffect(() => {
    if (lineIndex >= serviceLines.length) {
      if (!showButton) {
        setShowButton(true);
      }
      return;
    ...
    const activeLine = serviceLines[lineIndex];
  ...
  const renderTypedLine = (line, index) => {
    const isActiveLine = lineIndex === index && lineIndex < serviceLines.length;
  ...
  return (
    <div className='overlay-container mt-[-96px] pt-[96px]'>
      <div className='w-full max-w-[1240px] h-screen mx-auto px-4 text-left flex flex-col justify-center items-start overlay-content retro-display'>
        ...
        <p className={`${lineTextClass} py-4 h-[4.5rem] md:h-[5rem] w-full`}>
          {renderTypedLine(typedLines[2], 2)}
        </p>
        <p className={`${lineTextClass} py-4 h-[4.5rem] md:h-[5rem] w-full`}>
          {renderTypedLine(typedLines[3], 3)}
        </p>
